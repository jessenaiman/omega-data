import pytest
from fastapi.testclient import TestClient
from src.api.main import app

client = TestClient(app)

def test_specify_endpoint_contract():
    """
    Contract test for POST /specify endpoint
    Validates request/response schemas according to contract
    """
    # This test should initially fail since the endpoint is not implemented yet
    response = client.post("/specify", json={"feature_description": "test feature"})
    
    # Validate response structure based on contract
    assert response.status_code == 201  # Or 200 depending on implementation
    
    response_data = response.json()
    assert "branch_name" in response_data
    assert "spec_file_path" in response_data
    assert "status" in response_data
    assert response_data["status"] == "specification created"
    
    # Validate data types
    assert isinstance(response_data["branch_name"], str)
    assert isinstance(response_data["spec_file_path"], str)